...
main = defaultMain [
    bgroup "naive" [bench "Test Short (length: 5)" $ nf naiveIO benchmark1,
                    ...
                    bench "Test NaiveStress (length: 11)" $ nf naiveIO benchmark10
                    ],
    bgroup "backtrack" [bench "Test Short (length: 5)" $ nf reorderIO benchmark1,
                        ...
                        bench "Test Lock-Unlock (length: 14)" $ nf reorderIO benchmark7,
                        bench "Test More-Threads (length: 14)" $ nf reorderIO benchmark8,
                        bench "Test NaiveStress (length: 11)" $ nf reorderIO benchmark10,
